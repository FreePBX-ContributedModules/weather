...
//   Nerd Vittles ZIP Weather ver. 1.1, (c) Copyright Ward Mundy, 2006. All rights reserved.
...
if ($debug) :
  fputs($stdlog, "Nerd Vittles ZIP Weather ver. 1.1 (c) Copyright 2006, Ward Mundy. All Rights Reserved.\n\n" . date("F j, Y - H:i:s") . "  *** New session ***\n\n" ); 
endif ;
...
//new connect tosql db
$link = DBI->connect("DBI:mysql:$VARDB_database:$VARDB_server:$VARDB_port", "$VARDB_user", "$VARDB_pass")
    or die("Data base connection failed");
mysql_select_db("asterisk")
    or die("data base open failed");
...
$thetext = "<br>" ;
$start= strpos($value, $thetext);
$newvalue="This National Weather Service update provided for ". $city . ". Current local conditions at " . substr($value, $start);
...
if ($emaildebuglog) :
 system("mime-construct --to $email --subject " . chr(34) . "Nerd Vittles ZIP Weather ver. 1.1 Session Log" . chr(34) . " --attachment $log --type text/plain --file $log") ;
endif ;
...
//new to phrase the amportal.conf
